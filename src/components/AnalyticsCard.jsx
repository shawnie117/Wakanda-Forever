import { motion } from 'framer-motion'
import GlassCard from './GlassCard'

export default function AnalyticsCard({ title, value, subtitle, Icon, className = '' }) {
  return (
    <GlassCard className={`h-full ${className}`}>
      <motion.div whileHover={{ y: -2 }} className="flex flex-col items-start justify-between h-full">
        <div className="flex items-center gap-3 mb-4">
          {Icon && (
            <div className="p-3 rounded-xl bg-purple-500/15 transition-all shadow-[0_0_18px_rgba(147,51,234,0.35)]">
              <Icon className="text-purple-300" size={24} />
            </div>
          )}
          <h3 className="font-neo tracking-[0.08em] text-xs uppercase text-slate-300">
            {title}
          </h3>
        </div>
        <div className="flex flex-col">
          <motion.div
            initial={{ opacity: 0, scale: 0.5 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ delay: 0.2 }}
            className="text-3xl md:text-4xl font-semibold text-slate-50"
          >
            {value}
          </motion.div>
          {subtitle && <p className="text-slate-400 text-xs mt-2">{subtitle}</p>}
        </div>
      </motion.div>
    </GlassCard>
  )
}

